...
<body>
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
       ...
       <?php 
            ...
            $sql ="SELECT * FROM Posts ORDER BY Price ASC LIMIT $this_page_first_result, $results_per_page";
            $rs = $conn->query($sql);
            while ($row = $rs->fetch_assoc()) {
                ...
                $post_id = $row['Post_ID'];
                echo "<p><a href='filter_post_details.php?postid=$post_id' class='btn btn-primary' role='button'>View details &raquo;</a></p>";
                echo "</div>";
                echo "</div>";
                echo "<hr color='#eee'>";
            }
            ...
            for($page = 1 ; $page <= $number_of_pages ; $page++ ){
                echo"<a class = 'page' href='filter.php?page=" . $page . "'>" . $page . "</a> ";
            }
            echo "</div>";
        $conn->close();
       ?>
