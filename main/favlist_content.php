...
<head>
    ...
    <link href="css/favlist_content.css" rel="stylesheet">
...
<body>
    ...
    <!--div class="row"-->
       <?php 
            ...
            $results_per_page = 2;
            $sql ="SELECT * FROM Posts as a, Users as b, favlist as c WHERE (a.Post_ID = c.Post_ID AND b.ID = c.User_ID AND b.ID = '$user_id') ORDER BY a.TimeStamp DESC";
            //echo $sql;
            $rs = $conn->query($sql);
            if($rs->num_rows == 0){
            echo"<div class='class_div'>";
            echo "<p class='class_p'>No products in your Favorites!</p>";
            }
            else
            {
            ...
            $sql ="SELECT * FROM Posts as a, Users as b, favlist as c WHERE (a.Post_ID = c.Post_ID AND b.ID = c.User_ID AND b.ID = '$user_id') ORDER BY a.TimeStamp DESC LIMIT $this_page_first_result, $results_per_page";
            $rs = $conn->query($sql);
            while ($row = $rs->fetch_assoc()) {
                ...
                $post_id = $row['Post_ID'];
                echo "<p><a href='favlist_content_details.php?postid=$post_id' class='class_a_del' role='button'>Delete from <i class='material-icons' style='font-size:25px'>favorite</i></a></p>";
                echo "</div>";
                echo "</div>";
                echo "<hr color='#eee'>";
            }
            ...
            for($page = 1 ; $page <= $number_of_pages ; $page++ ){
                echo"<a class = 'page' href='favlist_content.php?page=" . $page . "'>" . $page . "</a> ";
            }
            echo "</div>";
        $conn->close();
